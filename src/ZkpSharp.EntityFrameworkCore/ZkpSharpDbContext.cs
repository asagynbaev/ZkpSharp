...
/// <summary>
/// EF Core <see cref="DbContext"/> holding ZkpSharp's normalized identity schema.
/// Provider-agnostic — consumers configure Postgres / SQL Server / SQLite at the
/// composition root by passing <see cref="DbContextOptions{ZkpSharpDbContext}"/>.
/// </summary>
/// <remarks>
/// Schema:
/// <list type="bullet">
...
/// </remarks>
public class ZkpSharpDbContext : DbContext
{
    public ZkpSharpDbContext(DbContextOptions<ZkpSharpDbContext> options)
        : base(options) { }

    /// <summary>For testing or subclass scenarios that pass a typed-options.</summary>
    protected ZkpSharpDbContext(DbContextOptions options) : base(options) { }
