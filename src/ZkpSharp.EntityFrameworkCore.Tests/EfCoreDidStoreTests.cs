...
{
    private static DidDocument SampleDocument(string id = "did:zkp:store-test-1")
    {
    ...
    public async Task Get_MissingDid_ReturnsNull()
    {
        ...
        var loaded = await store.GetAsync(new DidId("did:zkp:does-not-exist"));
        Assert.Null(loaded);
    }
    ...
    {
        using var fx = new SqliteFixture();
        var doc = SampleDocument("did:zkp:exists-check");
    ...
    {
        using var fx = new SqliteFixture();
        var doc = SampleDocument("did:zkp:wallet-update");
    ...
    {
        using var fx = new SqliteFixture();
        var doc = SampleDocument("did:zkp:root-test");
    ...
    {
        using var fx = new SqliteFixture();
        var doc = SampleDocument("did:zkp:channels");
        var withChannels = doc with
        {
            Bindings = new[]
            {
                new ChannelBinding
                {
                    ...
                    Issuer = new DidId("did:zkp:phone-issuer"),
        ...
        await using (var db = fx.CreateContext())
        {
            ...
            Assert.Equal("did:zkp:phone-issuer", loaded.Bindings[0].Issuer.Value);
        }
    }
}
