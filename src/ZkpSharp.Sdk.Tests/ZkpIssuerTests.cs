...
public class ZkpIssuerTests
{
    ...
    public void Issue_ProducesAttestationBoundToIssuerAndSubject()
    {
        ...
        var issuer = new ZkpIssuer(new DidId("did:zkp:my-issuer"), signer);

        var subject = new DidId("did:zkp:subject-1");
        var att = issuer.Issue(
            type: "phone_verified",
            subject: subject,
            payload: new AttestationPayload { Method = "test_provider" });

        Assert.Equal(new DidId("did:zkp:my-issuer"), att.Issuer);
    ...
    public void BuildRegistryRecord_ShapesIssuerRecord()
    {
        ...
        using var signer = new Ed25519IssuerSigner(priv);
        var issuer = new ZkpIssuer(new DidId("did:zkp:another-issuer"), signer);
    ...
    public void BuildRegistryRecord_CanMarkInactive()
    {
        var (priv, _) = Ed25519.GenerateKeypair();
        using var signer = new Ed25519IssuerSigner(priv);
        var issuer = new ZkpIssuer(new DidId("did:zkp:revoked-issuer"), signer);
    ...
    public void Issue_OverrideSchema_UsesProvided()
    {
        var (priv, _) = Ed25519.GenerateKeypair();
        using var signer = new Ed25519IssuerSigner(priv);
        var issuer = new ZkpIssuer(new DidId("did:zkp:issuer"), signer);

        var att = issuer.Issue(
            type: "custom",
            subject: new DidId("did:zkp:s"),
